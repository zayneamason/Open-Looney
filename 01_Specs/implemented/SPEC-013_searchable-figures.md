...
- Engine PR [#173](https://github.com/zayneamason/LunaEngineBetaV2.0/pull/173) merged 2026-07-26 as `b7c02cff` (enrichment hits promote to figure results). Implemented in `luna/substrate/aibrarian_engine.py` — `_v03_promote_figure_extraction`, `_v03_figure_snippet`, `_v03_figure_neighbours`, `_v03_attach_figure_context` — **not** in `luna.cartridge`, whose reader API has no runtime callers. First runtime consumer of `extraction_context_nodes`. `FIGURE_ENRICHMENT_TYPES` in `luna/cartridge/media.py` is the shared reference point. `PAGE_IMAGE_DPI = 150` and `PAGE_IMAGE_META_KEY` live in `luna/cartridge/media.py` alongside `DEFAULT_EMBED_MAX`; `is_page_image` / `is_page_image_meta` are the shared predicates (dict form for the builder's in-memory nodes, JSON form for enrichments reading back out of SQLite).
