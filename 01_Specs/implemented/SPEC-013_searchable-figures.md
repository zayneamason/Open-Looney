...
13. `figure_discourse` — **done** Engine PR [#170](https://github.com/zayneamason/LunaEngineBetaV2.0/pull/170) merge `fb8d81cc`: prev/next paragraph soft-links via `extraction_context_nodes`
...
| 2026-07-26 | **`visual_description` content may be vision-derived** (Engine PR #174, `c70d8937`). The type string, `extraction_sources` anchoring, and the non-empty-content validator are **unchanged and still frozen**; only what fills `content` varies. With the builder's opt-in `--figure-vision`, content is a vision-model description of the raster; otherwise it remains the caption/OCR rollup stub frozen on 2026-07-25. Readers MUST NOT assume either origin — there is no provenance marker distinguishing them, and `extraction_method` stays `rule` in both cases. Page-image figures remain enrichment-free per the row below. **⚠ Versioning question for the next wiki pass:** this amends the content semantics of a type in `implemented/`, which `WIKI_VERSIONING.md:58-61` may classify as MAJOR rather than MINOR. Not decided here. |
| 2026-07-26 | **Retrieval contract for enrichment types** (Engine PR #173, `b7c02cff`). `figure_discourse` / `visual_description` / `media_classification` remain FTS-matchable but MUST NOT be returned as evidence rows in their own right: a hit resolves through `extraction_sources` to the figure it anchors and is emitted as a **figure result**. Rationale: `figure_discourse.content` is by construction a verbatim copy of neighbour paragraphs the node index already carries, so returning it duplicates prose and cites no figure. Keeping the rows indexed preserves the retrieval paths that matter — neighbour prose, and the `src_hint`-derived `media_kind` that is often the only handle on a figure whose caption says nothing. Neighbour context is capped when carried. `media_kind` `other` is a no-cue default and is not surfaced. |
...
- Engine PR [#172](https://github.com/zayneamason/LunaEngineBetaV2.0/pull/172) merged 2026-07-26 as `465b784c` (scanned PDF page-as-image typing).
- Engine PR [#173](https://github.com/zayneamason/LunaEngineBetaV2.0/pull/173) merged 2026-07-26 as `b7c02cff` (enrichment hits promote to figure results).
- Engine PR [#174](https://github.com/zayneamason/LunaEngineBetaV2.0/pull/174) merged 2026-07-26 as `c70d8937` (`--figure-vision` → real `visual_description`). `luna/cartridge/figure_vision.py`; opt-in, one API call per figure; oversize rasters downscaled via the optional `vision` extra (Pillow), else skipped.

**Scope note (#174):** a vision description is *text in an extraction*, so it does
**not** enter `nodes_fts` or the MiniLM `embeddings` — those cover `paragraph` and
`section` `doc_nodes` only (verified: zero embeddings anchored to any figure). It
reaches retrieval through the extraction FTS and the #173 promotion path, so the
gain is **lexical**: a figure becomes findable by words describing its content
rather than only by its caption. Semantic and cross-modal figure search remain
unbuilt and still require vision embeddings — see Still deferred item 7. Implemented in `luna/substrate/aibrarian_engine.py` — `_v03_promote_figure_extraction`, `_v03_figure_snippet`, `_v03_figure_neighbours`, `_v03_attach_figure_context` — **not** in `luna.cartridge`, whose reader API has no runtime callers. First runtime consumer of `extraction_context_nodes`. `FIGURE_ENRICHMENT_TYPES` in `luna/cartridge/media.py` is the shared reference point. `PAGE_IMAGE_DPI = 150` and `PAGE_IMAGE_META_KEY` live in `luna/cartridge/media.py` alongside `DEFAULT_EMBED_MAX`; `is_page_image` / `is_page_image_meta` are the shared predicates (dict form for the builder's in-memory nodes, JSON form for enrichments reading back out of SQLite).
